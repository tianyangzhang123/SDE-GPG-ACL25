...
# 返回了 match_fn 函数对象
# 比如 result = cache_match(g) 此时result相当于存储了一个函数，在运行result(参数)时才会真实地运行match_fn
def cache_match(
    graph: gh.Graph,
...
# 递归地尝试将一组几何子句（clause）中的点映射到实际的几何点上
def try_to_map(
...
def match_generic(
  ...
  for clause in theorem.premise:
    if clause.name in ['ncoll', 'npara', 'nperp', 'sameside']:
      ...
      continue
    # 返回图中所有满足这个条件的内容，比如clause.name = 'para'，那么enum列表中获得所有平行的线段关系，每个列表元素是一个元组
...
# 在给定的几何图 g 中匹配所有定理（theorems），并返回每个定理的所有可能映射（mappings）
...
def bfs_one_level(
  ...
  # Step 3: add conclusions to graph. 添加结论到图中
  # 对于每个成功推导的定理，函数会将其结论添加到几何图 g 中。这一步会生成新的几何对象（如新的线段、角度等），并记录它们的依赖关系。
  # Note that we do NOT mix step 2 and 3, strictly going for BFS.
  added = []
  theorem_set = set()
  for theorem, mp_deps in theorem2deps:
    if theorem.name == 'midp_midp_para':
      if theorem.rule_name == 'r02':
        theorem_set.add(theorem.name+"_1")
      elif theorem.rule_name == 'r25': # original 06
        theorem_set.add(theorem.name+"_2")
      elif theorem.rule_name == 'r44': # original 25
        theorem_set.add(theorem.name+"_3")
    else:
      theorem_set.add(theorem.name)
  ...
  # 检查当前图是否满足目标条件。
  # 如果满足，返回当前添加的几何对象、空的代数推导结果和定理集合。
  if controller.goal:
    args = []
    ...
    if g.check(controller.goal.name, args):
      return added, {}, {}, branching, theorem_set
  ...
  # added 添加到图中的新几何对象；derives 推导出的代数关系；eq4s 等式关系；branching 分支数量；theorem_set 使用的定理集合
  return added, derives, eq4s, branching, theorem_set
