...
class Graph:
  ...
  def __init__(self):
    ...
    self.rconst = {}  # contains all constant ratios 常值比例
    self.aconst = {}  # contains all constant angles. 常值角度

    # self.halfpi存储了小于90度中的常值角度，同时给每个角度创建了节点存储角度值
    self.halfpi, _ = self.get_or_create_const_ang(1, 2)
    # self.vhalfpi存储了常值角度的值
    self.vhalfpi = self.halfpi.val

    self.atable = ar.AngleTable() # 角度关系
    self.dtable = ar.DistanceTable() # 距离关系
    self.rtable = ar.RatioTable() # 比例关系

    # to quick access deps. 短期内存？快速访问deps
    self.cache = {}
  ...
  def derive_algebra(
    ...
    # 分离等角和等比关系 等角、等比关系较多 会影响推导速度，单独处理
  ...
  def build_problem(
    ...
    if verbose:
      logging.info(pr.url)
      # 将Problem实例中的内容重新组合成problem文本
      logging.info(pr.txt())
    while not check:
      try:
        g = Graph() # 初始化图像信息
        added = [] # 存储图像中点线的全部依赖关系
        plevel = 0 # 存储图像中点的总数
  ...
  # 为一个节点创建并连接一个值节点，用于表示该节点的值（如长度、角度大小等）
  ...
  # 检测当前construction名字和点数、点的情况是否对得上
  ...
  def get_line_thru_pair_why(
    ...
    if p1.name.lower() > p2.name.lower():
      p1, p2 = p2, p1
    if (p1, p2) in self._pair2line:
      # TODO 实际上线段已经存在于_pair2line中，但是返回的对象总是与输入不匹配
      # print(self._pair2line[(p1, p2)])
  ...
  def add_perp(
    ...
    a, b, c, d = points[0],points[1],points[2],points[3]
    # a, b, c, d = points
    # TODO 到此为止abcd都处于正常范畴，但是经历了get_line_thru_pair_why函数后会返回相同的东西
    try:
      ab, why1 = self.get_line_thru_pair_why(a, b)
      cd, why2 = self.get_line_thru_pair_why(c, d)
    except:
  ...
  def add_clause(
    ...
    existing_points = self.all_points() # existing_points = Graph类中的point属性（列表）
    new_points = [Point(name) for name in clause.points]

    new_points_dep_points = set()
    new_points_dep = []

    # Step 1: check for all deps. 检查所有的依赖
    for c in clause.constructions: # 遍历clause中的每一个Construction
      cdef = definitions[c.name] # 找到当前Construction的name对应的Definition实例

      if len(cdef.construction.args) != len(c.args): # 如果长度不等，则规则适用的点数不对应【不一定是错误】
        if len(cdef.construction.args) - len(c.args) == len(clause.points): # 可能是省略了部分内容，如果符合长度设计，则将省略的部分补齐
          c.args = clause.points + c.args # 将对应的point补充到条件中
        else: # 报错
          correct_form = ' '.join(cdef.points + ['=', c.name] + cdef.args)
          raise ValueError('Argument mismatch. ' + correct_form)

      mapping = dict(zip(cdef.construction.args, c.args)) # 将定义中的点和条件中的点一一对应
      ...
      for d in cdef.deps.constructions:
        ...
        if not self.check(d.name, args): # check不匹配 则报错
    ...
    # 绘图自由度 都只有一个规则 完全自由-随便画 半自由-有固定点
    is_total_free = (
    ...
    # add these points to current graph.将生成的点添加到当前图中，并更新相关属性
    for p in new_points:
    ...
    # movement dependency: 更新点对象的基础依赖关系
    ...
    for c, deps in zip(clause.constructions, new_points_dep):
      ...
      self.additionally_draw(c.name, c_args) # 辅助线 or 附加线

      for points, bs in cdef.basics:
        if points: # 给所有的点做编号-先后画的顺序
    ...
    assert len(plevel_done) == len(new_points) # 确保所有点都构造完毕
    for p in new_points:
      p.basics = basics
