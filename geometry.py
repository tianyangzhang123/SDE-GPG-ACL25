...
class Node:
  ...
  d.merged_to = e 当前节点合并到的下一个节点
  d.rep = g 当前节点的最终代表节点
  d.merged_from = {a, b, c, d} 当前节点是从哪些节点合并而来的
  d.equivs = {a, b, c, d, e, f, g} 当前节点的等价类，包括所有与当前节点等价的节点
  """
  ...
  def __init__(self, name: str = '', graph: Any = None):
    ...
    self.edge_graph = {}
    # Edge graph: what other nodes is connected to this node. 与其他节点的连接关系
    # edge graph = {
    ...
    self.merge_graph = {}
    # Merge graph: history of merges with other nodes. 节点之间的合并历史
    ...
    self.num = None
    self.change = set()  # what other nodes' num rely on this node? 存储依赖于当前节点数值的其他节点
  ...
  def rep(self) -> Node:
    x = self
    while x.rep_by: # TODO：问题出在这里 但是回溯发现只有共线信息会导致线和线融合，所以这里为什么会在找bd线时找到ab
       x = x.rep_by
    return x
