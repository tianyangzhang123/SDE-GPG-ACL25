...
class Table:
  ...
  def __init__(self, const: str = '1'):
    self.const = const
    self.v2e = {}
    # 将变量v添加到v2e字典中，v2e用于映射变量名到其表达式
    self.add_free(const)  # the table {var: expression}

    # to cache what is already derived/inputted 缓存已经推导或输入的方程
    self.eqs = set()
    self.groups = []  # groups of equal pairs. 存储等价对

    # for why (linprog)
    self.c = []
    self.v2i = {}  # v -> index of row in A. 变量名映射到矩阵 A 的行索引
    self.deps = []  # equal number of columns. 表示矩阵 A 的列数
    self.A = np.zeros([0, 0])  # pylint: disable=invalid-name 表示系数矩阵，默认为一个空矩阵
    self.do_why = True
...
class AngleTable(GeometricTable):
  ...
  AngleTable类继承GeometricTable类，GeometricTable类继承Table类
  """

  def __init__(self, name: str = ''):
    name = name or 'pi'
    super().__init__(name)
    self.pi = self.const # 默认为 1
