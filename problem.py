...
"""Implements objects to represent problem, theorems, proofs, traceback."""
...
class Construction:
  ...
  def from_txt(cls, data: str) -> Construction:
    # 传入 eqangle b a b x b x b c
    data = data.split(' ')
    # Construction
    #     name =  eqangle
    #     args = [b,a,b,x,b,x,b,c]
  ...
  def txt(self) -> str:
    if self.args and isinstance(self.args[0], str):
      ...
      return ' '.join([self.name] + [i.name for i in list(self.args)])
    # return ' '.join([self.name] + list(self.args))


# Clause包含以下内容
#   points = [..,..,..] 存储point中的字母信息
#   nums = [..,..,..] 存储point中的数字信息，没有的话记为空数组
...
class Clause:
  ...
  def from_txt(cls, data: str) -> Clause:
    # 传入数据 a b c = ncoll a b c
    if data == ' =':
      return Clause([], [])
    points, constructions = data.split(' = ')
    return Clause(
        points.split(' '), # [a,b,c]
        [Construction.from_txt(c) for c in constructions.split(', ')], #Construction
    )
  ...
  def translate(self, mapping: dict[str, str]) -> Clause:
    ...
    for p in self.points:
      ...
      p0 = mapping.get(p, name) # 如果mapping中有p则直接用p，不然用name
      mapping[p] = p0 # 更新mapping
      points0.append(p0)
...
#   goal = Construction
# 如果translate设置为true，则clauses和goal会将名字部分转换成alphabet
class Problem:
  ...
  def from_txt_file(
    ...
    lines = [l for l in lines if l] # 在刚刚的基础上过滤掉空行
    # reshape(lines, 2)，将一维列表lines转换成二维列表，n*2列 分别对应url（题号）和problem（问题）
    data = [
        cls.from_txt(url + '\n' + problem, translate)
  ...
  def from_txt(cls, data: str, translate: bool = True) -> Problem:
    ...
    # TODO 不知道为什么只要运行完problem.translate() goal就会变成None
    if translate:
...
class Translation:
  ...
  @classmethod
  def from_txt_file(cls, fname: str, to_dict: bool = False) -> Translation:
    with open(fname, 'r') as f:
      lines = f.read()
    return cls.from_string(lines, to_dict)

  @classmethod
  def from_string(cls, string: str, to_dict: bool = False) -> Translation:
    lines = string.split('\n')
    data = [cls.from_txt('\n'.join(group)) for group in reshape(lines, 7)]
    if to_dict:
      return cls.to_dict(data)
    return data

  @classmethod
  def from_txt(cls, data: str) -> Translation:
    """Load translations from a str object."""
    construction, rely, deps, basics, numerics, desc, _ = data.split('\n')
    ...
    return Translation(dfn, desc)

  @classmethod
  def to_dict(cls, data: list[Translation]) -> dict[str, Translation]:
    return {d.definition.construction.name: d for d in data}

  def __init__(self, dfn: Definition, desc: str) -> Translation:
    self.definition = dfn
    self.desc = desc
...
def parse_rely(s: str) -> dict[str, str]:
  ...
  if not s:
    return result
  s = [x.strip() for x in s.split(',')] #用，划分成n个元素
  for x in s:
    a, b = x.split(':')
    a, b = a.strip().split(), b.strip().split() # a = x b = [a,b,c,x]
...
# Definition包含以下内容
#   construction = Construction 第一行数据制作的
#   rely = {[]:[.,.,.],...,...} 第二行数据制作的字典
#   deps = Clause 【Clause 包含两个性质 points[常规元素,...] construction[Construction,Construction,...]】
#   basics = [([],[Construction,Construction,...]),(...)]
#   numerics[Construction,Construction,...]
class Definition:
  ...
  def from_string(cls, string: str, to_dict: bool = False) -> Definition:
    lines = string.split('\n')
    data = [cls.from_txt('\n'.join(group)) for group in reshape(lines, 6)] # 将输入的行列表 lines 按照每组 6 行进行分组
    if to_dict:
      return cls.to_dict(data)
    return data
  ...
  def from_txt(cls, data: str) -> Definition:
    ...
    '''
    angle_bisector x a b c        construction
    x : a b c x                   rely
    a b c = ncoll a b c           deps
    x : eqangle b a b x b x b c   basics 用；分隔
    bisect a b c                  numerics
    '''
    basics = [] if not basics else [b.strip() for b in basics.split(';')]
    ...
    for bs in basics:
      ...
      if bs.strip():
        ...
        bs = []
      # levels 的元素 ([x],[Construction])
      levels.append((points, bs))

    # bisect a b c 存储最后一行的内容，并以，划分
    numerics = [] if not numerics else numerics.split(', ')
...
# Theorem包含以下内容
# name = premise+conclusion首词用_拼接
# premise = [Construction,Construction,...]
# conclusion = [Construction] 多次检验确保长度为1
# is_arg_reduce 记录premise和conclusion数量的比较结果
class Theorem:
  """Deduction rule."""
  ...
  def from_string(cls, string: str, to_dict: bool = False) -> Theorem:
    ...
    # 每一行存储为一个列表元素，并将每一行转换为Theorem类实例
    theorems = [l for l in theorems if l and not l.startswith('#')]
    theorems = [cls.from_txt(l) for l in theorems]
    # theorems = [Theorem,Theorem,...]

    for i, th in enumerate(theorems): # i是当前实例的索引，th是第i个Theorem实例
      th.rule_name = 'r{:02}'.format(i) # 给当前的Theorem实例添加rule_name属性，用rxx的方式排序

    if to_dict: # 转换成字典的形式
      result = {}
      for t in theorems:
        if t.name in result: # 避免名字重复，如果重复就在最后加一个_
          t.name += '_'
  ...
  def from_txt(cls, data: str) -> Theorem:
    # Theorem.premise = [Construction,Construction,...]
    # Theorem.conclusion = [Construction,Construction,...]
