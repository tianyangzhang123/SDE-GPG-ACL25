...
def saturate_or_goal(
  ...
  while len(level_times) < max_level:
    ...
    t = time.time()
    added, derv, eq4, n_branching, tset = dd.bfs_one_level(
        g, theorems, level, p, verbose=False, nm_check=True, timeout=timeout
    )
    theorem_set.append(tset)
    all_added += added
  ...
  return derives, eq4s, branching, all_added, theorem_set
...
def solve(
  ...
  while len(level_times) < max_level:
    dervs, eq4, next_branches, added, _ = saturate_or_goal(
